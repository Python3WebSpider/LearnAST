...
const code = fs.readFileSync("obfuscates/code3.js", "utf-8");
let ast = parse(code);
      ...
      fnPath.traverse({
        CallExpression: function (_path) {
          ...
          if (types.isBinaryExpression(retStmt.argument) && args.length === 2) {
            _path.replaceWith(
              types.binaryExpression(retStmt.argument.operator, args[0], args[1])
            );
          }
          // 逻辑运算
          else if (
            ...
            _path.replaceWith(
              types.logicalExpression(retStmt.argument.operator, args[0], args[1])
            );
          }
          // 函数调用
          else if (
